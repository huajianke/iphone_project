...
#include "../include/asn.h"           // program version
#include "../include/asntrace.h"      // trace definitions required in asnccp.h
#include "../include/asnmsgl.h"       // msglogger functions
#include "../include/asnmsgt.h"       // deprecated msgToLog functions
#include "../include/asnccp.h"        // parameter definitions
#include "../include/asnthred.h"      // thread functions
#include "../include/asncryptinit.h"  // cryptInit
#include "../include/asnbind.h"       // bindCommonLibFiles
#include "../include/asnsqlcf.h"      // genLogfileName, getTabId
#include "../include/asntxs.h"        // transmgr
#include "../include/asnreg.h"        // registration
#include "../include/asndecd.h"       // decodebase
#include "../include/asnthred.h"      // thread

#include "../include/asnwpctpcbmgr.h" // Transaction pattern control block manager.
...
//----------------------------------Start------------------------------------------
/*
	gloabls:	ManuStop :this is a global parm that informs all the threads that it 
				is time to stop
				
*/
static	int ManuStop = 0;
static  int g_rc = 0;
static  dbConn	*mainConn		=	NULL;
static	OrdList	mainThredList;
static	recoveryRes	*thredListRes	=	NULL;
static	asnLSN	startingLSN;
...
static	logReaderClient	*dbTableMgr; //Log reader interface
static	logReadStats	logStats;
static	trans	*pTrans;
static	trans	*wpTrans;
static	int sig = 1;
extern "C" void P(int s)
{
	if(s<=0)
...
extern "C" void V(int s)
{
	s++;
}
extern "C" void Exit()
{
	const char funcName[]="Exit";
...
{
	printf("Start to run thread getTrans!\n");
	const char funcName[]	=	"threadEntry_getTrans";
	int rc					=	AsnRcOk;
	
	Uint64	retrievedLogRecords = 0;

	printf("INFO WPC thread entry starts.\n");
	...
	printf("INFO completed initialization of global parms.\n");
	...
	printf("INFO Completed verifying log archiving.\n");
	...
	TPCBMgr.decodeMem	=	&decodeMem;
	printf("INFO Completed creating trans manager and log reader thread.\n");
	//Register tables
	{
	...
	printf("INFO Completed adding interested tables.\n");
	// Read the log an count transaction patterns.
	TPCBMgr.TotalTimesDumped = 0;
	int zxtixxx = 0;
	while((!g_rc)&&(!ManuStop)&&(!timeToQuit))
	{
		zxtixxx++;
		printf("The %d times to run the getTrans!\n",zxtixxx);
		P(sig);
		pTrans = NULL;
		...
		if(pTrans)
		{
			printf("After getTrans, pTrans = %d\n",pTrans);
		}
		else
		{
			printf("After getTrans, pTrans is NULL\n");
		}
		if(pAsnParm->getBool(DEBUG))
		{
	...
	printf("End of thread getTrans\n");
	return NULL;
}
extern "C" void *threadEntry_TPaDump(void *)
{
	printf("Start to run thread DUMP!\n");
	const char funcName[]= "threadEntry_TPaDump";
	int rc = AsnRcOk;
	while((!g_rc)&&(!ManuStop)&&(!timeToQuit))
	{
		if(wpTrans)
		{
			...
			rc = theTransMgr->delTrans(wpTrans);
			P(sig);
			wpTrans = NULL;
			V(sig);
			if(rc)
		...
		else if(g_rc == AsnRcWarnLogLimit)
		{
			g_rc = AsnRcOk;
		}
	...
	//Free all memory allocated for transaction pattern manager
	TPCBMgr.freeAll();
	printf("End of thread DUMP!\n");
	Exit();
	return NULL;
}
...
int main(int argc, char *argv[])
{
  ...
  int rc =AsnRcOk;
  int threadRc;
  pthread_t thrdID_getTrans,thrdID_TPaDUMP;
  pthread_attr_t	attr_getTrans,attr_TPaDUMP;
  size_t stack_size;
  pthread_attr_init(&attr_getTrans);
  pthread_attr_init(&attr_TPaDUMP);
  stack_size = asnThread_WORKER_STACK_SIZE;
  pthread_attr_setstacksize(&attr_getTrans,stack_size);
  pthread_attr_setstacksize(&attr_TPaDUMP,stack_size);
  threadRc = pthread_create(&thrdID_getTrans,&attr_getTrans,&threadEntry_getTrans,NULL);
  if(threadRc!=0)
  {
	  printf("There are some errors occured while creating thread threadEntry_getTrans\n");
  }
  else
  {
	  printf("Created thread threadEntry_getTrans successfully!\n");
  }
  threadRc = pthread_create(&thrdID_TPaDUMP,&attr_TPaDUMP,&threadEntry_TPaDump,NULL);
  if(threadRc!=0)
  {
	  printf("There are some errors occured while creating thread threadEntry_TPaDump\n");
  }
  else
  {
	  printf("Created thread threadEntry_TPaDump successfully!\n");
  }
  //threadEntry_getTrans(NULL);
  //threadEntry_TPaDump(NULL);
  while(!ManuStop)
  {
	  sleep(SLEEP_INTERVAL);
  }
  pthread_join(thrdID_getTrans,NULL);
  pthread_join(thrdID_TPaDUMP,NULL);
  mainThread.threadExit(g_rc);
  return g_rc;
}
...
void initDefaultCCPTparms(parmStruct ptr[CAP_PARAMETER_COUNT])
{
  ...
  //  Comment out following parm which is not contained in current code release
/*
