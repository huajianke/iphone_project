...
int main(int argc, char *argv[])
{
  ...
  asnLSN       startingLSN;
  MemoryBlock  decodeMem;

  TransPatternCBMgr &TPCBMgr = TransPatternCBMgr::getInstance();
  ...
  //----------------------------------------------------------------------------
  transmgr*        theTransMgr;                        // Transaction manager
  logrd*           logTxReader;                        // Log reader 
  ...
  //---------------------------------------------------------------------------
  {
    ...
    {
		i++;
		printf("This is the %d time to run the round\n",i);
      pTrans = NULL;   
	  ...
	  printf("At location1,TPCBMgr.TotalTimesDumped = %d\n",TPCBMgr.TotalTimesDumped);
	  printf("At location1,TPCBMgr.DumpCount = %d\n",TPCBMgr.DumpCount);
      if (pTrans)
      {
        //----------------------------------------------------------------------
        ...
        //----------------------------------------------------------------------
		printf("At location2,TPCBMgr.TotalTimesDumped = %d\n",TPCBMgr.TotalTimesDumped);
		printf("At location2,TPCBMgr.DumpCount = %d\n",TPCBMgr.DumpCount);
        Uint64 LatestLogTime    = TPCBMgr.getLatestLogTime();
		printf("At location3,TPCBMgr.TotalTimesDumped = %d\n",TPCBMgr.TotalTimesDumped);
		printf("At location3,TPCBMgr.DumpCount = %d\n",TPCBMgr.DumpCount);
		printf("At location3, LatestLogTime = %d\n",LatestLogTime);
		printf("At location3, TPCBMgr.DumpStartTimestamp = %d",TPCBMgr.DumpStartTimestamp);
		...
		printf("At location4,TPCBMgr.TotalTimesDumped = %d\n",TPCBMgr.TotalTimesDumped);
		printf("At location4,TPCBMgr.DumpCount = %d\n",TPCBMgr.DumpCount);
		printf("At location4, ClosestDumpPoint = %d\n",ClosestDumpPoint);
		printf("At location4, TPCBMgr.DumpStartTimestamp = %d",TPCBMgr.DumpStartTimestamp);
        if (pAsnParm->getBool(DEBUG)) {
