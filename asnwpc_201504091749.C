...
{
	printf("Strat to run thread getTrans()!\n");
	pthread_mutex_trylock(&g_mutex);
	if(logTxReader!=NULL)
	{
		printf("before....logTxReader=%d\n",logTxReader);
	}
	else
	{
		printf("before....logTxReader is NULL\n");
	}
	if(&decodeMem)
	{
		printf("before....&decodeMem=%d\n",&decodeMem);
	}
	else
	{
		printf("before....&decodeMem is NULL\n");
	}
	if(pTrans!=NULL)
	{
		printf("before....pTrans=%d\n",pTrans);
	}
	else
	{
		printf("before....pTrans is NULL\n");
	}
	mainrc = getTrans( logTxReader,    // The log reader to use
	...
	if(logTxReader!=NULL)
	{
		printf("after....logTxReader=%d\n",logTxReader);
	}
	else
	{
		printf("after....logTxReader is NULL\n");
	}
	if(&decodeMem)
	{
		printf("after....&decodeMem=%d\n",&decodeMem);
	}
	else
	{
		printf("after....&decodeMem is NULL\n");
	}
	if(pTrans!=NULL)
	{
		printf("after....pTrans=%d\n",pTrans);
	}
	else
	{
		printf("after....pTrans is NULL\n");
	}
...
//---------------------------------------------------------------------------
 /*
   ...
   */                
	  int ret;
	  pthread_t id;
	  pthread_attr_t zxattr;
	  pthread_attr_init(&zxattr);
	  size_t stack_size;
	  stack_size = asnThread_WORKER_STACK_SIZE;
	  pthread_attr_setstacksize(&zxattr, stack_size);
	  ret = pthread_attr_getstacksize(&zxattr,&stack_size);
	  if(ret != 0)
	  {
		  printf("There are some errors occured while getting stacksize\n");
	  }
	  else
	  {
		  printf("The size of the stack now , is %d B\n",stack_size);
	  }
	  
	  threadEntry_getTrans(NULL);/*-----------------------------------
	  ret = pthread_create(&id,&zxattr,&threadEntry_getTrans,NULL);
	  if(ret == 0)
	  {
		  printf("Created thread getTrans successfully!\n");
	  }
	  else
	  {
		  printf("There is something wrong with the thread creation!\n");
	  }
